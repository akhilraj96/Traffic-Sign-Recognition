...
class LaNet(tf.Module):
    ...
    def __init__(self, n_out=43, mu=0, sigma=0.1, learning_rate=0.001):
        ...
        self.model = tf.keras.Sequential([
            tf.keras.layers.Conv2D(
                6, (5, 5), activation='relu', input_shape=(32, 32, 1)),
    ...
    def accuracy(self, logits, labels):
        ...
        correct_predictions = tf.equal(
            predicted_classes, tf.cast(labels, tf.int64))
    ...
    def evaluate(self, x_data, y_data, batch_size=64):
        ...
        dataset = tf.data.Dataset.from_tensor_slices(
            (x_data, y_data)).batch(batch_size)
...
class VGGnet(tf.Module):
    ...
    def __init__(self, n_out=43, mu=0, sigma=0.1, learning_rate=0.001):
        ...
        self.model = tf.keras.Sequential([
            tf.keras.layers.Conv2D(
                32, (3, 3), activation='relu', padding='same', input_shape=(32, 32, 1)),
            tf.keras.layers.Conv2D(
    ...
    def accuracy(self, logits, labels):
        ...
        correct_predictions = tf.equal(
            tf.argmax(logits, 1), tf.argmax(y_onehot, 1))
        return tf.reduce_mean(tf.cast(correct_predictions, tf.float32))
    ...
    def evaluate(self, x_data, y_data, batch_size=64):
        ...
        for batch_x, batch_y in dataset:
            batch_x = tf.cast(batch_x, dtype=tf.float32)
            # Ensure batch_y has int32 data type
